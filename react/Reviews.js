      ...
      <div className="review mw8 center ph5">
              ...
              {this.state.percentage.map((percentage, i) => {
                return (
                  <li key={i} className="mv3">
                    <span className="dib w-10 v-mid">{5 - i} {(i < 4) ? "stars" : "star"}</span>
                    <div className="review__histogram--bar bg-white dib h2 w-90 v-mid">
                      <div className="review__histogram--bar-value h2 bg-yellow" style={{ width: percentage }}>
                      </div>
                    </div>
                  </li>
                )
          ...
          <div className="review__comments">
            <div className="review__comments_head">
              <h4 className="review__comments_title t-heading-4 bb b--muted-5 mb5 pb4">Reviewed by {this.state.count} {(this.state.count == 1) ? "customer" : "customers"}</h4>
              <div className="mb7">
                <Dropdown
              ...
              <div className="mv5">
                ...
                ? <a href={`/new-review?pr_page_id=${this.props.productQuery.product[this.props.data.getConfig.uniqueId]}&pr_merchant_id=${this.props.data.getConfig.merchantId}&pr_api_key=${this.props.data.getConfig.appKey}&pr_merchant_group_id=${this.props.data.getConfig.merchantGroupId}`}> Write a review </a>
                : null}
              ...
              return (
                <div key={i} className="review__comment bw2 bb b--muted-5 mb5 pb4">
                  <div className="review__comment--rating">
                    {[0, 1, 2, 3, 4].map((_, j) => {
                      return (
                        <svg className="mr3" key={j} xmlns="http://www.w3.org/2000/svg" width="14.737" height="14" fill={review.metrics.rating > j ? "#fc0" : "#eee"} viewBox="0 0 14.737 14"><path d="M7.369,11.251,11.923,14,10.714,8.82l4.023-3.485-5.3-.449L7.369,0,5.3,4.885,0,5.335,4.023,8.82,2.815,14Z" transform="translate(0)"/></svg> // se o review.metrics.rating for 4, preenche 4 estrelas
                      )
                    })}

                    <span>{review.metrics.rating}</span>
                  </div>
                  <h5 className="review__comment--user lh-copy mw9 t-heading-5 mv5">{review.details.headline}</h5>
                  <ul className="pa0">
                    {(review.badges.is_verified_buyer) ? (<li className="dib mr5"><IconSuccess /> Verified buyer</li>) : (null)}
                      <li className="dib mr5"><strong>Submitted</strong> {this.getTimeAgo(review.details.created_date)}</li>
                      <li className="dib mr5"><strong>By</strong> {review.details.nickname}</li>
                      <li className="dib"><strong>From</strong> {review.details.location}</li>
                  </ul>
                  <p className="t-body lh-copy mw9">{review.details.comments}</p>
                  <div>
                    <h5>Was this review helpful to you?</h5>
                    <Button disabled={review.disabled} variation="primary" size="small" onClick={() => this.voteReview(review.review_id, "helpful", i)}>
                      yes {review.metrics.helpful_votes}
                    </Button>

                    <Button disabled={review.disabled} variation="danger-tertiary" size="small" onClick={() => this.voteReview(review.review_id, "unhelpful", i)}>
                    ...
                    <Collapsible
                      ...
                      })}
                      {
                        ...
                        (<div className="w30">
                            ...
                            <p>{review.details.bottom_line}, I would {(review.details.bottom_line == "No") ? "not " : "" }recommend to a friend</p>
                          </div>) : (null)
                      }
                    </div>
                    </Collapsible>
                  ...
                  {review.media.length ?
                    <div className="review__comment-images mt6">
                      {review.media.map((item, i) => {
                        return (
                          <img className="w-20 db mb5" key={i} src={item.uri}></img>
                        )
                      })}
                    </div> : (null)
                  }
          ...
          <div className="review__paging">
            <Pagination
              currentItemFrom={1 + (this.state.paging.current_page_number - 1) * this.state.paging.page_size}
              currentItemTo={this.state.paging.current_page_number * this.state.paging.page_size}
              textOf="of"
              totalItems={this.state.paging.total_results}
              onNextClick={this.goToPage.bind(this, this.state.paging.current_page_number * 10)}
              onPrevClick={this.goToPage.bind(this, (this.state.paging.current_page_number - 2) * 10)}
            />
          </div>
      </div>
    ) : (<div className="review mw8 center ph5">
        ...
        <div className="review__comments">
          <div className="review__comments_head">
            ...
            <div className="review__comment bw2 bb b--muted-5 mb5 pb4">
              <h5 className="review__comment--user lh-copy mw9 t-heading-5 mv5">No reviews found!</h5>
            </div>
          </div>
        </div>
    </div>)
...
export default withApollo(withGetConfig(productReviews));
